...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
myManualCalibrator2::myManualCalibrator2(int pxWidth, int pxHeight, int hzRefreshRate, bool monitor) :
    ExCalibrator(pxWidth, pxHeight, hzRefreshRate, monitor)
{
	...
	m_fileCross = "img/whitecross.tga";
	m_fileStimulus = "img/redcross.tga";
...
void myManualCalibrator2::eventRender(unsigned int, CEOSData* Samples)
{
    ...
    if (Samples->samplesNumber > 0)
    {
       ...
       // print(CFontEngine::FONTS_ARIAL_BLACK_10, 0.0f, -(m_pxHeight / 2.0f) + 10, EOS_RENDER_MONITOR_FIRST, false, "x %f y %f", x, y);
...
void myManualCalibrator2::eventKeyboard(unsigned char key, int, int)
{
	...
	if (key == 'r' || key == 'R')
	{
		...
		/*
		long int ID[1] = { -1 };
		EAnalogOut(ID, 0, m_fakeH[m_cursorPoint], m_fakeV[m_cursorPoint]);
		*/
...
void myManualCalibrator2::eventJoypad()
{
    ...
    {
		CEnvironment::Instance()->outputMessage("R1 pressed - %f.", m_timerExp.getTime());
        m_cursorPoint++;
    ...
    {
		CEnvironment::Instance()->outputMessage("L1 pressed - %f.", m_timerExp.getTime());
        m_cursorPoint--;
        if (m_cursorPoint < 0)
        {
            m_cursorPoint = POINTS_NUM;
...
void myManualCalibrator2::finalize()
{
	...
	EDigitalOut(ID, 0, 0, 0, false); // TRACK1 set to High
	EDigitalOut(ID, 0, 1, 0, false); // BLINK1 set to Low

	EDigitalOut(ID, 0, 2, 0, false); // TRACK2 set to High
	EDigitalOut(ID, 0, 3, 0, false); // BLINK2 set to Low
	*/
}
...
void myManualCalibrator2::initialize()
{
	...
	/*
	m_fakeV = std::vector<float>({ 0, 0, 0, 2.5, 2.5, 2.5, 5, 5, 5});
	m_fakeH = std::vector<float>({ 0, 2.5, 5, 0, 2.5, 5, 0, 2.5, 5});

	long int ID[1] = { -1 };
	EAnalogOut(ID, 0, 0, 0);

	EDigitalOut(ID, 0, 0, 0, true); // TRACK1 set to High
	EDigitalOut(ID, 0, 1, 0, false); // BLINK1 set to Low

	EDigitalOut(ID, 0, 2, 0, true); // TRACK2 set to High
	EDigitalOut(ID, 0, 3, 0, false); // BLINK2 set to Low
	*/
 ...
 //   // Set the eye position stimulus (red cross)
	...
	//{
		m_planeStimulus = addObject(new CImagePlane(m_fileStimulus));
		m_planeStimulus->pxSetSize(30, 30);
 ...
 //   // Set the initial point coordinates
	...
	//{
		m_crossStimulus = addObject(new CImagePlane(m_fileCross));
		m_crossStimulus->pxSetSize(30, 30);
